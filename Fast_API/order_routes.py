...
from models import Pedidos, Usuario, ItemPedido, Adicionais, EnderecoEntrega, TipoPagamento, PrecoAdicional
...
async def remover_adicional(id_item_pedido: int, id_adicional: int, id_tamanho: int, session: Session =  Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    item = session.query(ItemPedido).filter(ItemPedido.id == id_item_pedido).first()
    adicional = session.query(PrecoAdicional).filter(PrecoAdicional.adicional_id == id_adicional, PrecoAdicional.tamanho_id == id_tamanho).first()    
...
async def adicionar_adicionais(id_pedido:int, id_item_pedido: int, id_adicional:int, id_tamanho: int, session: Session =  Depends(pegar_sessao), usuario: Usuario = Depends(verificar_token)):
    pedido =session.query(Pedidos).filter(Pedidos.id == id_pedido).first()
    item = session.query(ItemPedido).filter(ItemPedido.id == id_item_pedido).first()
    adicional = session.query(PrecoAdicional).filter(PrecoAdicional.adicional_id == id_adicional, PrecoAdicional.tamanho_id == id_tamanho).first()
